...
class Stats:
    def __init__(self, game: Game) -> None:
        ...
        self.game: Game = game
        self.windows = {"level":
                        self.Window("Level", 1400, 200, "level", game),
                        "score":
                        self.Window("Score", 1400, 400, "score", game),
                        "lives":
                        self.Window("Lives", 1400, 600, "lives", game),
                        "time":
            ...
            if self.stat != "lives":
                if (self.stat == "score" and
                   self.game.stats.stats["score"] > 999999999999999):
                    ...
                    self.game.display.show_text(
                        str(self.game.stats.stats[self.stat]),
                        self.x + 15, self.y + 67,
                        "left", "maze_screen")
            else:
                pac_w = self.game.display.images["pacman_3_right"].width + 13
                for i in range(self.game.stats.stats["lives"]):
                    self.game.display.add_to_bitmap(
                        "maze_screen",
                        "pacman_3_right",
                        self.x + pac_w * i + 15,
                        self.y + 70)
