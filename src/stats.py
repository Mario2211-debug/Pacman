...
class Stats:
    ...
    class Window:
        ...
        def show_window(self) -> None:
            self.game.display.show_text(self.text, self.x, self.y, "left", "maze_matrix")
            self.update_window()
        ...
        def update_window(self) -> None:
            ...
            if self.stat != "lives":
                self.game.display.show_text(str(self.game.stats.stats[self.stat]), self.x, self.y + 75, "left", "maze_matrix")
            else:
                for i in range(self.game.stats.stats["lives"]):
                    ...
                    self.game.display.add_to_matrix("maze_matrix", "pacman_right", self.x + (self.game.display.images["pacman_right"].width + 10) * i, self.y + 75)
